...
// Add these AMENITIES METHODS to your existing Database class:

// Amenities Management Methods
public function getAllAmenitiesCategories() {
    $sql = "SELECT * FROM amenities_categories WHERE Status = 'active' ORDER BY Display_Order, Category_Name";
    try {
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    } catch (PDOException $e) {
        error_log("getAllAmenitiesCategories Error: " . $e->getMessage());
        return [];
    }
}

public function getAmenitiesByCategory($category_id) {
    $sql = "SELECT * FROM amenities WHERE Category_ID = ? AND Status = 'active' ORDER BY Display_Order, Amenity_Name";
    try {
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute([$category_id]);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    } catch (PDOException $e) {
        error_log("getAmenitiesByCategory Error: " . $e->getMessage());
        return [];
    }
}
...
public function getAllAmenities() {
    $sql = "SELECT a.*, ac.Category_Name 
            FROM amenities a 
            JOIN amenities_categories ac ON a.Category_ID = ac.Category_ID 
            WHERE a.Status = 'active' 
            ORDER BY ac.Display_Order, a.Display_Order, a.Amenity_Name";
    try {
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    } catch (PDOException $e) {
        error_log("getAllAmenities Error: " . $e->getMessage());
        return [];
    }
}

public function getSpaceAmenities($space_id) {
    $sql = "SELECT sa.Amenity_ID, a.Amenity_Name, a.Icon, ac.Category_Name, ac.Category_ID
            FROM space_amenities sa
            JOIN amenities a ON sa.Amenity_ID = a.Amenity_ID
            JOIN amenities_categories ac ON a.Category_ID = ac.Category_ID
            WHERE sa.Space_ID = ?
            ORDER BY ac.Display_Order, a.Display_Order";
    try {
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute([$space_id]);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    } catch (PDOException $e) {
        error_log("getSpaceAmenities Error: " . $e->getMessage());
        return [];
    }
}

public function addAmenityToSpace($space_id, $amenity_id) {
    // Check if already exists
    $check_sql = "SELECT COUNT(*) FROM space_amenities WHERE Space_ID = ? AND Amenity_ID = ?";
    try {
        $check_stmt = $this->pdo->prepare($check_sql);
        $check_stmt->execute([$space_id, $amenity_id]);
        
        if ($check_stmt->fetchColumn() == 0) {
            $sql = "INSERT INTO space_amenities (Space_ID, Amenity_ID) VALUES (?, ?)";
            $stmt = $this->pdo->prepare($sql);
            return $stmt->execute([$space_id, $amenity_id]);
        }
        return false; // Already exists
    } catch (PDOException $e) {
        error_log("addAmenityToSpace Error: " . $e->getMessage());
        return false;
    }
}

public function removeAmenityFromSpace($space_id, $amenity_id) {
    $sql = "DELETE FROM space_amenities WHERE Space_ID = ? AND Amenity_ID = ?";
    try {
        $stmt = $this->pdo->prepare($sql);
        return $stmt->execute([$space_id, $amenity_id]);
    } catch (PDOException $e) {
        error_log("removeAmenityFromSpace Error: " . $e->getMessage());
        return false;
    }
}

public function getSpaceAmenitiesGrouped($space_id) {
    $amenities = $this->getSpaceAmenities($space_id);
    $grouped = [];
    ...
    foreach ($amenities as $amenity) {
        ...
        $grouped[$category_id]['amenities'][] = $amenity;
    }
    
    return $grouped;
}

public function addNewAmenityCategory($category_name, $icon = 'fas fa-star', $display_order = 0) {
    $sql = "INSERT INTO amenities_categories (Category_Name, Icon, Display_Order) VALUES (?, ?, ?)";
    try {
        $stmt = $this->pdo->prepare($sql);
        return $stmt->execute([$category_name, $icon, $display_order]);
    } catch (PDOException $e) {
        error_log("addNewAmenityCategory Error: " . $e->getMessage());
        return false;
    }
}

public function addNewAmenity($category_id, $amenity_name, $icon = 'fas fa-check', $description = '', $display_order = 0) {
    $sql = "INSERT INTO amenities (Category_ID, Amenity_Name, Icon, Description, Display_Order) VALUES (?, ?, ?, ?, ?)";
    try {
        $stmt = $this->pdo->prepare($sql);
        return $stmt->execute([$category_id, $amenity_name, $icon, $description, $display_order]);
    } catch (PDOException $e) {
        error_log("addNewAmenity Error: " . $e->getMessage());
        return false;
    }
}

public function updateAmenityCategory($category_id, $category_name, $icon, $display_order) {
    $sql = "UPDATE amenities_categories SET Category_Name = ?, Icon = ?, Display_Order = ? WHERE Category_ID = ?";
    try {
        $stmt = $this->pdo->prepare($sql);
        return $stmt->execute([$category_name, $icon, $display_order, $category_id]);
    } catch (PDOException $e) {
        error_log("updateAmenityCategory Error: " . $e->getMessage());
        return false;
    }
}

public function updateAmenity($amenity_id, $amenity_name, $icon, $description, $display_order, $category_id) {
    $sql = "UPDATE amenities SET Amenity_Name = ?, Icon = ?, Description = ?, Display_Order = ?, Category_ID = ? WHERE Amenity_ID = ?";
    try {
        $stmt = $this->pdo->prepare($sql);
        return $stmt->execute([$amenity_name, $icon, $description, $display_order, $category_id, $amenity_id]);
    } catch (PDOException $e) {
        error_log("updateAmenity Error: " . $e->getMessage());
        return false;
    }
}

public function deleteAmenityCategory($category_id) {
    // Check if category has amenities
    $check_sql = "SELECT COUNT(*) FROM amenities WHERE Category_ID = ? AND Status = 'active'";
    try {
        $check_stmt = $this->pdo->prepare($check_sql);
        $check_stmt->execute([$category_id]);
        
        if ($check_stmt->fetchColumn() > 0) {
            return false; // Cannot delete category with active amenities
        }
        
        $sql = "DELETE FROM amenities_categories WHERE Category_ID = ?";
        $stmt = $this->pdo->prepare($sql);
        return $stmt->execute([$category_id]);
    } catch (PDOException $e) {
        error_log("deleteAmenityCategory Error: " . $e->getMessage());
        return false;
    }
}

public function deleteAmenity($amenity_id) {
    // Check if amenity is used in any space
    $check_sql = "SELECT COUNT(*) FROM space_amenities WHERE Amenity_ID = ?";
    try {
        $check_stmt = $this->pdo->prepare($check_sql);
        $check_stmt->execute([$amenity_id]);
        
        if ($check_stmt->fetchColumn() > 0) {
            return false; // Cannot delete amenity that's in use
        }
        
        $sql = "DELETE FROM amenities WHERE Amenity_ID = ?";
        $stmt = $this->pdo->prepare($sql);
        return $stmt->execute([$amenity_id]);
    } catch (PDOException $e) {
        error_log("deleteAmenity Error: " . $e->getMessage());
        return false;
    }
}
