  ...
  <script>
    ...
    function renderAvailableUnits(units) {
      ...
      units.forEach(space => {
        ...
        const photoUrls = photoFields.map(f => space[f]).filter(f => f).map(f => `uploads/unit_photos/${f}`);
        // Card
        let card = `<div class="col-lg-4 col-md-6 animate-on-scroll"><div class="card unit-card">`;
        if (photoUrls.length) {
          card += `<div style='position:relative;'><img src='${photoUrls[0]}' class='card-img-top' alt='${escapeHtml(space.Name)}' style='cursor:pointer;' data-bs-toggle='modal' data-bs-target='#${photoModalId}'><span class='badge bg-primary position-absolute top-0 end-0 m-2' style='z-index:2;'>${photoUrls.length}/6</span></div>`;
        } else {
        ...
        card += `<div class='card-body'><h5 class='card-title fw-bold'>${escapeHtml(space.Name)}</h5><p class='unit-price'>₱${Number(space.Price).toLocaleString()} / month</p><p class='card-text text-muted'>Premium commercial space in a strategic location.</p><div class='d-flex justify-content-between align-items-center mb-3'><span class='unit-type'>${escapeHtml(space.SpaceTypeName)}</span><small class='unit-location'>${escapeHtml(space.City)}</small></div>`;
    ...
    fetchAvailableUnits();
    setInterval(fetchAvailableUnits, 5000);
